  ...
  return (
    ...
    <div className="py-16 grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1  backdrop-blur-lg bg-black  bg-opacity-70 text-white w-full p-4 relative justify-center">
      <div className="flex flex-col space-y-2 items-center">
        <NavLink to={"/"} className="footer-img flex items-center space-x-1">
          <img
          ...
          <div to={"/"} className="flex flex-col justify-center relative space-y-1">
            <span className="[font-family:'Montserrat',Helvetica] font-semibold text-[2rem] tracking-[-2px]   text-white text-opacity-85 leading-6 text-center">
              Sports Board
            </span>
            <span className="w-[100%] border self-center border-white"></span>
            <span className="[font-family:'Montserrat',Helvetica] font-semibold text-[1.575rem] tracking-[-1.5px]  text-white text-opacity-85  text-center leading-6">
              IIIT Bhagalpur
            </span>
          </div>
        </NavLink>
      ...
      <div className="mx-2 grid w-full py-5 sm:py-0 grid-cols-2 justify-center">
        {links.map((col, index) => {
          return (
            <ul  key={`col-${index}`}>
              {col.map((link, index) => {
